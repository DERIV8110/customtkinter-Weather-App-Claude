...
response3 = requests.get(ip)
sdata3 = response3.json()
latitude2 = sdata3['lat']
longitude2 = sdata3['lon']
#print("temp data:",sdata3)

ip_lat_lon = f"https://api.open-meteo.com/v1/forecast?latitude={latitude2}&longitude={longitude2}&hourly=temperature_2m"
response4 = requests.get(ip_lat_lon)
sdata4 = response4.json()
#print("temp data:",sdata2)

Name_city = customtkinter.CTkLabel(home,text=sdata3['city'])
Name_city.grid(row=2,column=0)

temp_data2 = sdata4["hourly"]["temperature_2m"]
min_temp_label2 = customtkinter.CTkLabel(home,text=min(temp_data2))
min_temp_label2.grid(row=2,column=3)                                                # f" with {} in a continous string separates variables
                                                                      #and string
max_temp_label2 = customtkinter.CTkLabel(home,text=max(temp_data2))
max_temp_label2.grid(row=3,column=3)

def fetch_data():
       global min_temp_label2
       global max_temp_label2
       global Name_city
...
#ip_butt = customtkinter.CTkButton(home, text="Get IP", command=fetch_ip)
#ip_butt.grid(row=4,column=1)
